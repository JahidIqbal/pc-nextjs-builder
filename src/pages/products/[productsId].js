            ...
            <ul>
              {data.reviews.map((review, index) => (
                <li key={index} className="mt-4">
                  <strong className="text-xl">{review.username}</strong>
                  <br />
                  <div className="mt-2">
                    <ReactStars
                      count={5}
...
export const getStaticProps = async ({ params }) => {
  ...
  const categoriesId = params.productsId;
  const res = await fetch(
    `http://localhost:3004/featuredProducts/${productId}`
  );
  const data = await res.json();
